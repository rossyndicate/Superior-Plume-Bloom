...
```{r}
dirs = c('eePlumB/2_data_ingestion/data/in', 
...
For now, grab data in lindsay's labels folder

```{r}
files <- drive_ls(as_id(unlist(strsplit(val.sheet$`Drive Folder Link`[8], '\\?'))[1]))
pwalk(list(files$id, paste0(val.sheet$Initials[8], '_', files$name), rep(in_dir, nrow(files))), driveDown)
```
...
```{r}
#get the folder info
raw_loc = drive_ls(as_id(project_folder$id), pattern = 'eePlumB_raw_data', recursive = T, overwrite = T)
