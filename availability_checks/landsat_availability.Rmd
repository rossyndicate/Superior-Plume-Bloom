...
ls = ee.ImageCollection(l4.merge(l5).merge(l7).merge(l5).merge(l4))
...
for y in years:
  ...
  for d in range(0, len(uniqueDates), 1):
    ...
    oneDate = ls_aoi.filter(ee.Filter.date(ee.Date(date),ee.Date(datePlus1)))
  ...
  file_out = ee.batch.Export.table.toDrive(
    collection = ls_area, 
    selectors = ['date', 'wrs', 'area', 'p_area'],
