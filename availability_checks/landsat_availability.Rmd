...
#### getDate: Function to grab date from a list

```{python}
def getDate(i):
  date = uniqueDates.get(i)
  return date

```

#### getLength: Function to grab length of date

```{python}
def getLength(ee_list):
  length = ee_list.length().getInfo()
  return length
```
