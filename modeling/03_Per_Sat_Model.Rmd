...
This script develops and applies Gradient Tree Boost Models to Landsat and Sentinel mosaic-ed images.
...
We need to rescale B11 and B12 for the Sentinel data. While this will not change any downstream 'outlier' handling, it will [hopefully] help with the wonky application of Sentinel models since the range is so different for those bands.
...
We'll go through and apply the Landsat and Sentinel separately due to the differences in bands.
...
aoi_sc_area = aoi_sc_area.getInfo()
...
allAreas_l5_aoi = newStack_l5.map(calcArea_aoi)
allAreas_l7_aoi = newStack_l7.map(calcArea_aoi)
allAreas_l8_aoi = newStack_l8.map(calcArea_aoi)
allAreas_l9_aoi = newStack_l9.map(calcArea_aoi)

allAreas_l5_nosc = newStack_l5.map(calcArea_nosc)
allAreas_l7_nosc = newStack_l7.map(calcArea_nosc)
allAreas_l8_nosc = newStack_l8.map(calcArea_nosc)
allAreas_l9_nosc = newStack_l9.map(calcArea_nosc)

allAreas_l5_sc = newStack_l5.map(calcArea_sc)
allAreas_l7_sc = newStack_l7.map(calcArea_sc)
allAreas_l8_sc = newStack_l8.map(calcArea_sc)
allAreas_l9_sc = newStack_l9.map(calcArea_sc)

```

And then, sadly, these individual aoi geometries are too big to merge, so you have to export them each individually
...
export_summary_sen_aoi = (ee.batch.Export.table.toDrive(
  ...
  description = 'quick_gradientTreeBoost_sen2_stack_v' + v_date,
  folder = 'eePlumB_classification',
  fileFormat = 'csv'))
...
export_summary_sen_nosc = (ee.batch.Export.table.toDrive(
  ...
  selectors = ['missDate', 'classified', 'cloud', 'openWater', 'lightNSSed','OSSed', 'dNSSed', 'area_ha', 'extent'],
  description = 'quick_gradientTreeBoost_sen2_nosc_stack_v' + v_date,
  folder = 'eePlumB_classification',
  fileFormat = 'csv'))
...
export_summary_sen_Sc = (ee.batch.Export.table.toDrive(
  ...
  selectors = ['missDate', 'classified', 'cloud', 'openWater', 'lightNSSed','OSSed', 'dNSSed', 'area_ha', 'extent'],
  description = 'quick_gradientTreeBoost_sen2_sc_stack_v' + v_date,
  folder = 'eePlumB_classification',
  fileFormat = 'csv'))
