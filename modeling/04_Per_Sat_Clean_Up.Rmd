...
rs_subset <- rs_subset %>%
  mutate(mission = if_else(grepl('LAND', missDate),
         ...
         date = if_else(grepl('LAND', missDate),
...
```{r}
write_csv(rs_subset, paste0('data/output/GTB_by_mission_pass_v', v_date, '.csv'))
```
...
drive_upload(paste0('data/output/GTB_by_mission_pass_v', v_date, '.csv'), as_id(upload_to$id[1]))
```
