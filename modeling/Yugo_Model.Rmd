...
In order to access the GEE API, we need to authenticate our identity. To do this, you must type `earthengine authenticate` in the terminal window. When your browser states 'You are now authenticated with the gcloud CLI', the authentication is complete. This authentication is valid for 7 days.
...
allAreas = newStack.map(calcArea)
...
Because the data storage types are different (int32 and float64), we have to export the GTB output and the band data images separately.
...
```{r wrap, echo = F}
knitr::wrap_rmd('models/Yugo_Model.Rmd', width = 80, backup = NULL)
```
